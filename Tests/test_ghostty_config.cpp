...
TEST(GhosttyConfigTest, DarkenByOneIsBlack) {
    auto out = Config::Darken({ 255, 255, 255 }, 1.0);
    EXPECT_EQ(out.r, 0);
    EXPECT_EQ(out.g, 0);
    EXPECT_EQ(out.b, 0);
}
...
TEST(GhosttyConfigTest, DarkenByHalfHalvesEachChannel) {
    auto out = Config::Darken({ 200, 100, 50 }, 0.5);
    EXPECT_EQ(out.r, 100);
    EXPECT_EQ(out.g, 50);
    EXPECT_EQ(out.b, 25);
}
...
TEST(GhosttyConfigTest, DarkenLeavesZerosAtZero) {
    auto out = Config::Darken({ 0, 0, 0 }, 0.4);
    EXPECT_EQ(out.r, 0);
    EXPECT_EQ(out.g, 0);
    EXPECT_EQ(out.b, 0);
}
...
TEST(GhosttyConfigTest, DividerFromLightBackgroundDarkensBy8Percent) {
    ...
    auto out = Config::DeriveDividerFromBackground({ 255, 255, 255 });
    EXPECT_EQ(out.A, 255);
    EXPECT_EQ(out.R, 234);
    EXPECT_EQ(out.G, 234);
    EXPECT_EQ(out.B, 234);
...
TEST(GhosttyConfigTest, DividerFromDarkBackgroundDarkensBy40Percent) {
    ...
    // darken => 30 * 0.6 = 18 -> 18.
    auto out = Config::DeriveDividerFromBackground({ 0x1e, 0x1e, 0x1e });
    EXPECT_EQ(out.A, 255);
    EXPECT_EQ(out.R, 18);
    EXPECT_EQ(out.G, 18);
    EXPECT_EQ(out.B, 18);
...
TEST(GhosttyConfigTest, DividerIsAlwaysOpaque) {
    ...
    auto fromLight = Config::DeriveDividerFromBackground({ 240, 240, 240 });
    auto fromDark  = Config::DeriveDividerFromBackground({ 30, 30, 30 });
...
TEST(GhosttyConfigTest, ColorFromCopiesChannelsAndForcesAlpha) {
    auto out = Config::ColorFrom({ 0x12, 0x34, 0x56 });
    EXPECT_EQ(out.A, 255);
