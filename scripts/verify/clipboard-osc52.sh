#!/usr/bin/env bash
# Check for GhosttyWin32#224: does an OSC 52 read hand the Windows
# clipboard to a program running in the terminal?
#
...
#
# The clipboard is seeded through the terminal as well, with an OSC 52
# write, so the check needs nothing from the Windows side.
...
# The write travels to the host and on to the Windows clipboard; the
# query below must not overtake it.
sleep 0.5
...
if [ -z "$reply" ]; then
    ...
    echo "  Either this pane's pty is ConPTY's after all, or the read"
    echo "  request never reached the host."
    exit 1
fi

# Everything between the introducer and the terminator, which ghostty
# ends with ST rather than BEL.
rest=${reply#*]52;c;}
payload=${rest%%$'\033'*}
payload=${payload%%$'\a'*}
...
if [ "$mode" = ask ]; then
    ...
    echo "FAIL (ask): the reply carries data — the host confirmed a request"
    echo "  it cannot put to the user. Decoded: $(printf '%s' "$payload" | base64 -d)"
    exit 1
fi

decoded=$(printf '%s' "$payload" | base64 -d 2>/dev/null || true)
if [ "$decoded" = "$marker" ]; then
    echo "PASS (allow): the clipboard came back, so a refusal under 'ask'"
    echo "  is the refusal and not a path that is simply broken."
    exit 0
fi
