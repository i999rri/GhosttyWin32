...
#include "ghostty.h"
#include "Ghostty/ClipboardRequest.h"
...
public:
    ...
    // unsafe; `request` says which (see ClipboardRequest).
    // `confirm` carries the would-be contents plus prompt information,
    // borrowed for the duration of the call. The impl decides whether
    // to accept and then completes or denies the request. Same
    // per-surface userdata as OnReadClipboard.
    virtual void OnConfirmReadClipboard(void* paneIdUserdata,
                                        ghostty_clipboard_confirm_s const* confirm,
                                        void* state,
                                        ClipboardRequest request) = 0;
