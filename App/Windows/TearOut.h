...
// the source window's inherited state, move the tab across, place the
...
// inherited state — is handed in as `spawnHost` (App, which owns the
// window list, provides it).
class TearOut {
public:
    using SpawnHost = std::function<MainWindow*(WindowState::Inherited const& inherited)>;
